...
class Image extends BaseController
{
    ...
    public function create($id)
    {
        ...
        if ($file->getSizeByUnit("mb") > 2) {
            return redirect()->back()->with("errors", ["File too large"]);
        }

        if (!in_array($file->getMimeType(), ["image/png", "image/jpeg"])) {
            return redirect()->back()->with('errors', ["Invalid file format"]);
        }
        ...
        try {
            service("image")->withFile($fullPath)->fit(200, 200, "center")->save($fullPath);
    ...
    public function delete($id)
    {
        ...
        if ($article->image && is_file($path)) { // Comprobación adicional
            unlink($path);
        }
